...
def get_new_name(old_name: str) -> (str, str):
    ...
    formated_index = re.search(
        r"[ ._-]{0,3}[Ss]\d{1,2}[ ]{0,1}[Ee]\d{1,2}", current_name
    )
    if formated_index:
...
def get_remote(
    sftp_client: paramiko.sftp_client.SFTPClient, path, dirs=True
) -> List[str]:
...
def get_finished_downloads(path: Path, unfinished: List[str]) -> List[Path]:
    ...
    finished_files = [
        f
        for f in chain(*[path.rglob(ft) for ft in file_types])
        if f not in unfinished_files
    ]
